...
class FT_Dataset(Dataset):
    def __init__(self, ft_file, batch_size, max_seq_length, 
                 ...
                 prefix_cursor=1000000, infix_cursor=2000000, vocab_path=None):
        ...
        # Initialize encoder if vocab_path is provided and encoder is available
        self.encoder = None
        if vocab_path is not None and ENCODER_AVAILABLE:
            try:
                self.encoder = get_encoder(vocab_path)
                print(f"Initialized encoder from {vocab_path}")
            except Exception as e:
                print(f"Warning: Could not load encoder from {vocab_path}: {e}")
                self.encoder = None
        
        # Check if we need tokenization (if first sample has string context)
        self.needs_tokenization = False
        if len(self.ft_samples) > 0:
            sample = self.ft_samples[0]
            if isinstance(sample[0], str) or isinstance(sample[1], str):
                self.needs_tokenization = True
                if self.encoder is None and ENCODER_AVAILABLE:
                    print("Warning: Data contains strings but no encoder available. Trying default vocab path.")
                    # Try default vocab path
                    default_vocab = os.path.join(os.path.dirname(os.path.dirname(ft_file)), 'vocab')
                    if os.path.exists(default_vocab):
                        try:
                            self.encoder = get_encoder(default_vocab)
                            print(f"Using encoder from default path: {default_vocab}")
                        except:
                            pass
    ...
    def __getitem__(self, item):
        ...
        # Tokenize if context/completion are strings
        if self.needs_tokenization and self.encoder is not None:
            if isinstance(context, str):
                bos = 50256  # GPT-2 BOS token
                context_bpes, _ = self.encoder.encode(context)
                context = context_bpes + [bos]
            if isinstance(completion, str):
                eos = 50256  # GPT-2 EOS token
                completion_bpes, _ = self.encoder.encode(' ' + completion)
                completion = completion_bpes + [eos]
        elif isinstance(context, str) or isinstance(completion, str):
            raise ValueError(
                "Data contains strings but no encoder available. "
                "Please provide vocab_path or pre-tokenize the data."
            )
